...
lint:
  use:
    ...
    - PACKAGE_LOWER_SNAKE_CASE  # Allow PascalCase package names
breaking:
  use:
    - FILE
```

**Key Lint Rules Applied:**
- `RPC_REQUEST_RESPONSE_UNIQUE` - Each RPC has dedicated request/response types
- `RPC_REQUEST_STANDARD_NAME` - Request messages follow naming conventions
- `RPC_RESPONSE_STANDARD_NAME` - Response messages follow naming conventions
- All other STANDARD rules for consistent protobuf style
...
| Dedicated response types | Each RPC has unique request/response messages |
| Type safety | Wrapper types for streaming (e.g., `StreamReadResponse`) |
| Clear field semantics | `snake_case` for JSON, proper enum values |
